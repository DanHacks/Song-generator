#!/usr/bin/env bash
# Start SautiGen backend + frontend and expose them through a public
# Cloudflare quick tunnel (great for M-Pesa callbacks and sharing).
#
...
stop_all() {
  for f in "$PID_DIR"/backend.pid "$PID_DIR"/frontend.pid "$PID_DIR"/tunnel.pid; do
    if [ -f "$f" ]; then
...
(cd "$ROOT/frontend" && setsid nohup npm run dev -- --port "$FRONTEND_PORT" \
  </dev/null >"$LOG_DIR/vite.log" 2>&1 & echo $! >"$PID_DIR/frontend.pid")
...
echo "  Local API:       http://localhost:$BACKEND_PORT/api/health"
